...
onMounted(async () => {
  await materiasPrimasStore.fetchMateriasPrimas();
});
...
const filteredItems = computed(() => {
  ...
  return materiasPrimasStore.materiasPrimas.filter((item) => {
    return item.materia?.toLowerCase().includes(q);
  });
});
...
const saveItem = async () => {
  ...
  if (modalMode.value === "create") {
    await materiasPrimasStore.createMateriaPrima(payload);
  } else {
  ...
  <div class="font-sans text-[#1C1C1C]">
    <main class="w-full">
      <header class="flex flex-col md:flex-row md:items-start md:justify-between mb-10 gap-4">
        <div>
          <h1 class="text-3xl font-bold text-black tracking-tight">Gestão de Matérias Primas</h1>
          <p class="text-gray-500 mt-1">Administre os tipos de materiais</p>
        </div>
        <button
        ...
        >
          <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
            <h3 class="text-lg font-bold text-[#1C1C1C]">
              {{ modalMode === "create" ? "Nova Matéria Prima" : "Editar Matéria Prima" }}
            </h3>
