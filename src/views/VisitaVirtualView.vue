...
<template>
  <div class="min-h-screen bg-gray-100 flex flex-col font-sans text-gray-900">
    ...
    <!-- A NavBar continua fixa no topo (z-50) -->
    <NavBar />

    <!-- Área Principal -->
    <!-- 
      CORREÇÕES:
      1. Reduzi padding-top (pt-2) para diminuir o espaço vazio.
      2. Mantive justify-start para o conteúdo subir.
    -->
    <main 
      class="flex-1 flex flex-col items-center w-full transition-all duration-500"
    ...
    >
      ...
      <!-- 
        ...
        - Modo Janela: Restaurei as bordas grossas (border-gray-800) para simular o monitor.
          Usei borda inferior mais grossa (border-b-[24px]) para fazer o "queixo" da tela.
        - Modo Tela Cheia: Remove bordas e fixa abaixo da navbar.
      -->
      <div 
        class="relative transition-all duration-500 ease-in-out bg-black"
        :class="[
          isMaximized 
      ...
      >
        ...
        <div class="absolute top-4 right-4 z-50">
          ...
          >
            <svg v-if="isMaximized" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <span class="max-w-0 overflow-hidden group-hover:max-w-xs transition-all duration-500 ease-in-out whitespace-nowrap text-[10px] font-bold uppercase tracking-widest">
      ...
      <!-- PÉ DO MONITOR (BASE) -->
      <!-- Só aparece se não estiver maximizado -->
      <div v-if="!isMaximized" class="flex flex-col items-center w-full animate-fade-in">
        <!-- Haste -->
        <div class="w-32 h-16 bg-gradient-to-b from-gray-700 to-gray-800 shadow-inner mx-auto relative z-10 -mt-1"></div>
        <!-- Base Plana -->
