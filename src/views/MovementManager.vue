...
const saveReserva = async () => {
  await reservaStore.createReserva(currentItem.value)
...
<template>
  <div class="flex min-h-screen bg-gray-50 font-sans text-gray-800">
    ...
    <div class="flex-1 flex flex-col md:ml-64 transition-all duration-300">
      <main class="py-8 px-6 max-w-7xl mx-auto w-full">
        ...
        <div class="overflow-hidden rounded-lg border border-gray-100 bg-white shadow-sm">
          <table class="min-w-full divide-y divide-gray-100">
            ...
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Fluxo</th>
            ...
            <tr v-for="reserva in reservasFiltradas" :key="reserva.id" class="hover:bg-gray-50 transition-colors">
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ formatarData(reserva.data_movimentacao)
                }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="font-medium text-sm">{{ reserva.item_data.nome }}</div>
                <div class="text-xs text-gray-500">#{{ reserva.item_data.acervo.numero_acervo }}</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm flex items-center gap-2">
          <span class="px-2 py-0.5 rounded text-xs font-medium inline-block bg-gray-100">
            {{ reserva.local_origem_data.nome_local }}
          </span>
                <span class="text-gray-400 font-bold">→</span>
                <span class="px-2 py-0.5 rounded text-xs font-medium inline-block bg-white border">
            {{ reserva.local_destino_data.nome_local }}
          </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="font-medium text-sm text-gray-900">{{ reserva.responsavel.name }}</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="font-medium text-sm text-gray-700">{{ reserva.tipo_movimento }}</div>
              </td>
            </tr>
            </tbody>
          </table>
        ...
        <div v-if="showReservaModal"
          ...
          <div class="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden animate-fade-in">
            <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
              ...
              <button @click="showReservaModal = false" class="text-gray-400 hover:text-gray-600">
            ...
            <div class="p-6 space-y-4">
              <div>
                <div>
                  ...
                  <select v-model="currentItem.item"
                    ...
                    <option v-for="item in itensAcervoStore.itensAcervo" :key="item.id" :value="item.id">
                      {{ item.nome }}
                    </option>
                  </select>
                </div>
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Local de origem</label>
                <select v-model="currentItem.local_origem"
                        class="w-full rounded-lg border border-gray-300 p-2.5 focus:border-[#0F766E] focus:ring-1 focus:ring-[#0F766E] outline-none">
                  <option v-for="l in localizacoesStore.localizacoes" :key="l.id" :value="l.id">
                    {{ l.nome_local }}
                  </option>
                </select>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Local de destino</label>
                <select v-model="currentItem.local_destino"
                        class="w-full rounded-lg border border-gray-300 p-2.5 focus:border-[#0F766E] focus:ring-1 focus:ring-[#0F766E] outline-none">
                  <option v-for="l in localizacoesStore.localizacoes" :key="l.id" :value="l.id">
                    {{ l.nome_local }}
                  </option>
              ...
              <div>
                ...
                <select v-model="currentItem.tipo_movimento"
                        class="w-full rounded-lg border border-gray-300 p-2.5 focus:border-[#0F766E] focus:ring-1 focus:ring-[#0F766E] outline-none">
                  <option disabled value="">Tipo de movimento</option>
                  <option v-for="t in reservaStore.movimentoOptions" :key="t.value" :value="t.value">
                    {{ t.label }}
                  </option>
                </select>
              </div>
