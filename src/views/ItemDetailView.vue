...
const carregarItem = async () => {
  ...
  if (itemEncontrado) {
    ...
    let imgBase = itemEncontrado.img || 'https://via.placeholder.com/500?text=Sem+Imagem';
...
// Redireciona para o acervo filtrando pela coleção deste item
const voltarParaCategoria = () => {
  if (item.value && item.value.colecao) {
    ...
    // Fallback caso não tenha coleção definida
...
<template>
  <div class="min-h-screen bg-white font-sans text-gray-900 pb-20">
    ...
    <div v-else-if="!item" class="flex flex-col justify-center items-center h-[60vh]">
      ...
      <button @click="router.push('/acervototal')" class="text-sm text-gray-500 hover:text-black underline">Voltar ao Acervo</button>
    </div>
        ...
        <div class="lg:col-span-7">
          <div class="lg:sticky lg:top-24">
            ...
            >
              ...
              />
              <!-- Navegação Imagem -->
              <button @click.stop="anteriorImagem" v-if="item.imagens.length > 1" class="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/90 hover:bg-white rounded-full flex items-center justify-center shadow-md opacity-0 group-hover:opacity-100 transition-opacity z-20 text-black">←</button>
              <button @click.stop="proximaImagem" v-if="item.imagens.length > 1" class="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/90 hover:bg-white rounded-full flex items-center justify-center shadow-md opacity-0 group-hover:opacity-100 transition-opacity z-20 text-black">→</button>
              <div class="absolute top-4 right-4 bg-white/90 p-2 rounded-full shadow-sm pointer-events-none z-10">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" /></svg>
              </div>
            </div>

            <!-- MINIATURAS -->
            <div v-if="item.imagens.length > 1" class="flex gap-2 sm:gap-3 overflow-x-auto pb-2 scrollbar-hide justify-center lg:justify-start">
              <button 
            ...
            <p class="text-[10px] text-gray-400 mt-2 text-center lg:text-left">© Museu Sambaqui de Joinville / Foto: Documentação Técnica</p>
          </div>
        </div>
        ...
        <div class="lg:col-span-5 flex flex-col gap-8 sm:gap-12">
          ...
          <header>
            <div class="flex flex-wrap items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
              <span class="px-2 py-1 bg-black text-white text-[10px] font-bold uppercase tracking-widest rounded-sm">{{ item.colecao }}</span>
              <span class="text-[10px] font-bold uppercase tracking-widest text-gray-500 border border-gray-200 px-2 py-1 rounded-sm">{{ item.materia }}</span>
            </div>
            <h1 class="font-serif text-3xl sm:text-4xl md:text-5xl text-gray-900 leading-tight mb-4 sm:mb-6">{{ item.titulo }}</h1>
          ...
          <!-- TABELAS DE DADOS -->
          <section>
            <h3 class="text-xs font-bold uppercase tracking-widest text-gray-400 mb-4 border-b border-gray-100 pb-2">Características</h3>
            <dl class="grid grid-cols-1 gap-y-3 sm:gap-y-4 text-sm">
              <div class="grid grid-cols-3"><dt class="text-gray-500 font-medium">Denominação</dt><dd class="col-span-2 text-gray-900 font-serif italic">{{ item.subtipo }}</dd></div>
              <div class="grid grid-cols-3"><dt class="text-gray-500 font-medium">Técnica</dt><dd class="col-span-2 text-gray-900">{{ item.tecnica }}</dd></div>
              <div class="grid grid-cols-3"><dt class="text-gray-500 font-medium">Dimensões</dt><dd class="col-span-2 text-gray-900">{{ item.dimensoes_detalhadas }}</dd></div>
            </dl>
          </section>

          <section>
            <h3 class="text-xs font-bold uppercase tracking-widest text-gray-400 mb-4 border-b border-gray-100 pb-2">Histórico & Local</h3>
            <dl class="grid grid-cols-1 gap-y-3 sm:gap-y-4 text-sm">
              <div class="grid grid-cols-3"><dt class="text-gray-500 font-medium">Datação</dt><dd class="col-span-2 text-gray-900">{{ item.datacao }}</dd></div>
              <div class="grid grid-cols-3"><dt class="text-gray-500 font-medium">Procedência</dt><dd class="col-span-2 text-gray-900">{{ item.procedencia }}</dd></div>
            </dl>
            <div class="mt-4 bg-gray-50 p-4 rounded text-sm flex justify-between items-center">
                <span class="text-gray-500">Local Atual</span>
                <span class="font-mono font-bold text-blue-800 bg-blue-50 px-2 py-1 rounded border border-blue-100">{{ item.localizacao }}</span>
            </div>
          </section>

          <section v-if="item.descricao">
            <h3 class="text-xs font-bold uppercase tracking-widest text-gray-400 mb-4 border-b border-gray-100 pb-2">Análise</h3>
      ...
      <section v-if="currentRelatedItems.length > 0" class="border-t border-gray-200 pt-12 sm:pt-16 mb-16">
        <div class="text-center mb-8 sm:mb-10">
          ...
          <div class="inline-flex bg-gray-100 p-1 rounded-full mt-4">
            <button @click="activeTab = 'colecao'" class="px-4 sm:px-6 py-2 text-[10px] sm:text-xs font-bold uppercase tracking-widest rounded-full transition-all" :class="activeTab === 'colecao' ? 'bg-white text-black shadow-sm' : 'text-gray-500'">Coleção</button>
            <button @click="activeTab = 'materia'" class="px-4 sm:px-6 py-2 text-[10px] sm:text-xs font-bold uppercase tracking-widest rounded-full transition-all" :class="activeTab === 'materia' ? 'bg-white text-black shadow-sm' : 'text-gray-500'">Material</button>
          </div>
        </div>
        <div class="relative max-w-[1400px] mx-auto sm:px-12 group/arrows">
          <button @click="scrollCarrossel(-1)" class="hidden sm:flex absolute left-0 top-1/2 -translate-y-1/2 w-10 h-10 z-10 items-center justify-center bg-white border border-gray-200 rounded-full shadow-md hover:bg-black hover:text-white transition-all active:scale-95">&larr;</button>
          <div ref="scrollContainer" class="flex gap-4 sm:gap-6 overflow-x-auto scrollbar-hide snap-x snap-mandatory pb-8 pt-2 px-4 sm:px-0 scroll-smooth">
            <div v-for="(relItem, i) in currentRelatedItems" :key="i" class="shrink-0 w-full sm:w-[calc(50%-12px)] lg:w-[calc(25%-18px)] snap-start">
              <AcervoCard :item="relItem" class="h-full shadow-sm hover:shadow-md transition-shadow border border-gray-100 rounded-sm" />
            </div>
          </div>
          <button @click="scrollCarrossel(1)" class="hidden sm:flex absolute right-0 top-1/2 -translate-y-1/2 w-10 h-10 z-10 items-center justify-center bg-white border border-gray-200 rounded-full shadow-md hover:bg-black hover:text-white transition-all active:scale-95">&rarr;</button>
        </div>
      </section>
