...
const itensProcessados = computed(() => {
  return itensAcervoStore.itensAcervoFiltrados.filter((item) => {
    ...
    const superStringItem = [
      ...
      item.materia_prima?.materia || item.materia_prima?.nome || item.materia_prima,
      ...
      <main class="flex-1 pl-0 lg:pl-10 mt-6 lg:mt-0">
        ...
        <div class="mb-6">
          <div class="relative">
            ...
            <button
              @click="realizarBusca"
              class="absolute inset-y-0 right-0 px-4 text-white bg-teal-600 hover:bg-teal-700 rounded-r-lg transition-colors"
            >
              Buscar
            </button>
          </div>
