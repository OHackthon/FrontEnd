  ...
  <div class="flex min-h-screen bg-gray-50 font-sans text-[#1C1C1C]">
    ...
    <main class="flex-1 py-8 px-6 md:ml-64 w-full">
      ...
      <header class="flex flex-col md:flex-row md:items-start md:justify-between mb-10 gap-4">
        <div>
          <h1 class="text-3xl font-bold text-black tracking-tight">Gestão de Categorias do Acervo</h1>
          <p class="text-gray-500 mt-1">Administre as categorias dos itens do seu acervo</p>
        </div>
        <button 
      ...
      <div class="overflow-hidden rounded-lg border border-gray-100 bg-white shadow-sm">
        ...
        <div v-else-if="filteredCategorias.length === 0" class="p-8 text-center text-gray-500">
          <svg class="mx-auto h-12 w-12 text-gray-400 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
          </svg>
          <p class="text-lg font-medium">Nenhuma categoria encontrada</p>
          <p class="text-sm">{{ searchQuery ? 'Tente um termo de busca diferente' : 'Comece adicionando uma nova categoria' }}</p>
        </div>
        ...
        <table v-else class="min-w-full divide-y divide-gray-100">
          ...
          <tbody class="bg-white divide-y divide-gray-50">
            <tr v-for="categoria in filteredCategorias" :key="categoria.id" class="hover:bg-gray-50 transition-colors">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div class="flex justify-end gap-2">
                  <button 
                    ...
                    class="text-[#0F766E] hover:text-[#0F3D3E] transition-colors p-1 rounded hover:bg-[#0F766E]/10"
    ...
    <div v-if="modalStore.confirmDeleteModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
        <div class="p-6">
