...
const filteredItems = computed(() => {
  ...
  return localizacoesStore.localizacoes.filter((item) => {
    const matchesSearch =
      item.nome_local?.toLowerCase().includes(q) ||
...
const saveItem = async () => {
  ...
  if (modalMode.value === "create") {
    await localizacoesStore.createLocalizacao(payload);
  } else {
  ...
  <div class="font-sans text-[#1C1C1C]">
    <main class="w-full">
      <header class="flex flex-col md:flex-row md:items-start md:justify-between mb-10 gap-4">
        <div>
          <h1 class="text-3xl font-bold text-black tracking-tight">Gestão de Localizações</h1>
          <p class="text-gray-500 mt-1">Administre os locais de armazenamento</p>
        </div>
        <button
        ...
        >
          <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
            <h3 class="text-lg font-bold text-[#1C1C1C]">
              {{ modalMode === "create" ? "Nova Localização" : "Editar Localização" }}
            </h3>
          ...
          <div class="p-6 space-y-4">
            <div>
              ...
              <input
                v-model="currentItem.nome_local"
                type="text"
            ...
            <div>
              ...
              <input
                v-model="currentItem.estado"
                type="text"
                class="w-full rounded-lg border border-gray-300 p-2.5 focus:border-[#0F766E] focus:ring-1 focus:ring-[#0F766E] outline-none"
              />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Capacidade Estimada</label>
              <input
                v-model="currentItem.capacidade_estimada"
                type="number"
                class="w-full rounded-lg border border-gray-300 p-2.5 focus:border-[#0F766E] focus:ring-1 focus:ring-[#0F766E] outline-none"
              />
            </div>
